    ...
    std::unique_ptr<control_msgs::msg::JointJog>& joint)
{
    ...
    twist->twist.linear.z = -1 * (buttons[XBOX360_BTN_LT] - buttons[XBOX360_BTN_RT]);
...
{   
    ...
    switch (joystick_type_) {
        ...
        case JOYSTICK_XBOX360_WIRELESS: // xbox360 wireless
            if (msg->axes.size() != 4 || msg->buttons.size() != 17){
                ...
                std::cout << "Enter mismatch" << std::endl;
                return;
            }
    ...
    if (pub_twist) {
        ...
        // std::cout << "Publish twist msg" << std::endl;
    }
    else {
        ...
        joint_pub_->publish(std::move(joint_msg));
        // std::cout << "Publish joint msg" << std::endl;
    }
}
