...
class DetectionService(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('visualization_en', False)   # visualize (cv etc.)
        self.declare_parameter('detection_topic_en', True)  # publish to topic for each frame received
        self.declare_parameter('match_objects_en', False)   # match objects using vector (from a given list)
        ...
        if self.match_objects_en:
            ...
            self.n_target_object = 12
            self.target_object_str = ['Bluemoon', 'Oreo', 'Lays', 'Nongfu', 'Cola', 'Shuyuan', 'Sprite', 'Fanta', 'Libai', 'Franzzi', 'Shampoo', 'Bread']
            self.target_image_path = [f'/home/zzt/tk23_ws/src/tk23_vision/src/object_detection/object_detection/images/{self.target_object_str[idx]}' for idx in range(self.n_target_object)]
            self.target_vector_path = [f'/home/zzt/tk23_ws/src/tk23_vision/src/object_detection/object_detection/vectors/{self.target_object_str[idx]}.npy' for idx in range(self.n_target_object)]
            self.target_vec = []
            for i in range(self.n_target_object):
                if not os.path.exists(self.target_vector_path[i]):
                    ...
                    self.target_vec.append(img_vec)

                else:
                    self.target_vec.append(np.load(self.target_vector_path[i], allow_pickle=True))
    ...
    def detection_segmentation(self,
        ...
        real_sz = img.shape[0], img.shape[1]
        ...
        # Now the real image is img[:real_sz[0], :real_sz[1]]
        ...
        if request_img:
            ret_img = img[:real_sz[0], :real_sz[1]], img_pt[:, :, 2], []
        ...
        for res in results:
            ...
            for i in range(len(boxes.cls)):
                ...
                y1, x1, y2, x2 = res.boxes.xyxy[0]
                # bbox = x1, y1, x2, y2
                bbox = min(int(x1), real_sz[0] - 1), min(int(y1), real_sz[1] - 1), min(int(x2), real_sz[0] - 1), min(int(y2), real_sz[1] - 1)

                if tracking_fast_mode and 'person' not in cls:
                    continue

                mask_obj = masks[i].data.cpu().numpy().squeeze()
                # match the segment to the point cloud
                mask_obj = mask_obj[:real_sz[0], :real_sz[1]]
                ...
                if match_obj_cls:
                    ...
                    for i in range(self.n_target_object):
                        similarity = self.cos_similarity(img_vec.unsqueeze(0), self.target_vec[i]).max()
